"""
The paved-path suite: small COMPLETE aspalchemy programs, each run through the
...
text, proving the library wires together end to end. The diagnostics tests
close the loop the other way: a gringo message names the Python line that
authored the offending statement.
"""
...
from aspalchemy import SUP, ASPProgram, Choice, Field, GroundingError, Optimum, Predicate, Sum, Variable, pool
...
def test_e2e_forbid_filter() -> None:
    ...
    program.choose(Choice(Pick(x=X), condition=Cand(x=X)))
...
def test_e2e_aggregate_sum_constraint() -> None:
    ...
    program.choose(Choice(Take(v=V), condition=Coin(v=V)))
...
def test_e2e_grounding_diagnostic_names_authoring_line() -> None:
    # A body-only atom draws gringo's "does not occur in any rule head"
    # info; the raised error names the when() line that authored the rule.
    P = Predicate.define("p", ["x"])
    Q = Predicate.define("q", ["x"])
    program = ASPProgram()
    ...
    program.when(Q(x=1)).derive(P(x=1))  # the very next line: lineno + 1
    with pytest.raises(GroundingError, match="does not occur") as excinfo:
        program.solve()
    message = str(excinfo.value)
    assert "generated by" in message
    assert f"test_end_to_end.py:{lineno + 1}" in message


def test_e2e_raw_asp_interior_line_maps_to_the_call() -> None:
    # gringo's info fires on the SECOND line of the raw block; a raw_asp
    # element claims every one of its rendered lines, so the "generated by"
    # note still lands on the raw_asp() call itself.
    Base = Predicate.define("base", ["x"], show=False)
    Out = Predicate.define("out", ["x"])
    Lonely = Predicate.define("lonely", ["x"], show=False)
    program = ASPProgram()
    frame = inspect.currentframe()
    assert frame is not None
    lineno = frame.f_lineno
    program.raw_asp("base(1).\nout(X) :- base(X), lonely(X).", predicates=[Base, Out, Lonely])
    with pytest.raises(GroundingError, match="does not occur") as excinfo:
        program.solve()
    message = str(excinfo.value)
    assert "generated by" in message
...
def test_e2e_clean_program_solves_with_locations_on() -> None:
    # Location capture is on by default and invisible on the happy path:
    # the default render carries no location comments and solving works.
    Item = Predicate.define("item", ["x"], show=False)
    Keep = Predicate.define("keep", ["x"])
    program = ASPProgram()
    X = Variable("X")
    program.fact(Item(x=1), Item(x=2), Item(x=3))
    program.when(Item(x=X), X < 3).derive(Keep(x=X))
