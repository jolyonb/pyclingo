...
a deliberate aspalchemy-only rejection (the singleton lint, the
...
from aspalchemy.program_elements import render_body_terms
...
def ok(head: Term | None, body: Sequence[Term]) -> None:
    ...
    assert _gringo_grounds(head, body), f"aspalchemy accepted a rule gringo rejects: {_rule_text(head, body)}"
...
def bad(head: Term | None, body: Sequence[Term], match: str, *, gringo_rejects: bool) -> None:
    ...
    a deliberate aspalchemy-only rejection of text gringo grounds.
    """
...
def test_conditional_literal_head_var_not_in_condition_is_global() -> None:
    ...
    # is local to the literal); aspalchemy rejects it anyway — a deliberate
