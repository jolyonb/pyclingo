...
def test_undeclared_shown_atoms_fail_loudly() -> None:
    ...
    with pytest.raises(ValueError, match="never declared to aspalchemy: bar/1"):
        program.solve()
...
def test_undeclared_raw_atoms_fail_loudly_in_mixed_programs() -> None:
    ...
    with pytest.raises(ValueError, match="never declared to aspalchemy: hidden_atom/1"):
        program.solve()
...
def test_exhaustive_declaration_catches_modelless_violations() -> None:
    ...
    with pytest.raises(ValueError, match="never declared to aspalchemy: helper/1"):
        unsat.solve()
    ...
    with pytest.raises(ValueError, match="never declared to aspalchemy: maybe/1"):
        never_true.solve()
...
def test_show_declares_as_fully_as_predicates() -> None:
    # The contract is "aspalchemy must know the class", by any door:
...
def test_raw_show_term_forms_get_a_teaching_error() -> None:
    # #show term forms emit arbitrary non-atom output aspalchemy cannot
...
def test_external_rejected_with_teaching() -> None:
    ...
    # aspalchemy verb speaks: left unassigned it is false and rules through
...
def test_unrepresentable_string_values_diagnose_the_atom() -> None:
    # gringo handles escaped strings fine; aspalchemy has no escaping support,
