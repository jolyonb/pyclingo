...
def test_optimize_under_assumptions() -> None:
    ...
    assert unassumed.cost == (1,)  # nothing persists
...
def test_maximize_optimum_reports_negated_cost() -> None:
    ...
    picks.add(Pick(x=2))
    picks.add(Pick(x=3))
    program.choose(picks.at_least(1).at_most(1))
...
def test_sequential_guard_covers_descent() -> None:
    ...
    with pytest.raises(RuntimeError, match="still open"):
        grounded.optimize()
    steps.close()
    grounded.optimize()  # the grounding is free for the next search
...
def test_uniqueness_via_the_certified_stream() -> None:
    ...
    assert unique.optima is not None
...
def test_multi_priority_bound_is_priority_keyed() -> None:
    ...
    picks.add(Pick(x=2))
    picks.add(Pick(x=3))
    program.choose(picks.at_least(1).at_most(2))
...
def test_optima_mode_never_leaks_between_searches() -> None:
    ...
    assert everything.optima is not None and len(everything.optima) == 3
    ...
    assert again.optima is not None and len(again.optima) == 3
...
def test_bound_keys_are_best_effort_hints() -> None:
    ...
    assert applied.cost == (1,)
    assert [m.cost for m in applied.path] == [(1,)]  # tier-0 bound still pruned
    grounded.optimize(bound={0: 1})  # full-cover key: applies
    grounded.optimize(bound=1)  # single tier: bare int fine
