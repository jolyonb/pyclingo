...
rendered by aspalchemy and evaluated by clingo, must match Python's evaluation
of the same tree.
...
from aspalchemy import ASPProgram, Compl, Number, Predicate
...
def build_tree(rng: random.Random, depth: int) -> tuple[Any, Any]:
    """
    Returns (aspalchemy expression or Number, python int) for the same tree.
