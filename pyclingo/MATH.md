...
`tests/pyclingo/test_arithmetic.py` verifies each construct against clingo's
actual evaluation.
...
You do not need that table to read pyclingo's output. Python builds the
...
pyclingo renders your expression tree faithfully; clingo then evaluates it
...
2. **Negative exponents.** Python's `2 ** -1` is `0.5` — a float, a type
   neither clingo nor pyclingo has. Clingo evaluates `2**(-1)` to `0`.
...
4. **Division by zero deletes the ground instance.** Raw clingo drops any
   ...
   message. pyclingo's default `stop_on_log_level=INFO` promotes that to a
