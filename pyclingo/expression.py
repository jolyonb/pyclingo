...
def Equals(left: Term | int, right: Term | int) -> Comparison:
    """
    Creates an ASP comparison representing left == right.

    Args:
        left: The left-hand term
        right: The right-hand term

    Returns:
        Comparison: An ASP comparison representing equality

    Example:
        X = Variable("X")
        Y = Variable("Y")
        equals_expr = Equals(X, Y)  # Creates X = Y in ASP
    """
    return Comparison(left, ComparisonOperator.EQUAL, right)


def NotEquals(left: Term | int, right: Term | int) -> Comparison:
    """
    Creates an ASP comparison representing left != right.
    ...
    Returns:
        Comparison: An ASP comparison representing inequality

    Example:
        X = Variable("X")
        Y = Variable("Y")
        not_equals_expr = NotEquals(X, Y)  # Creates X != Y in ASP
    """
    return Comparison(left, ComparisonOperator.NOT_EQUAL, right)
