...
*Translation table: every clingo construct → its spelling here or its refusal link; CI-pinned by a trailing assert block.*
...
that owns it, and the [Receipts](#receipts) section asserts the load-bearing
renderings, so if a table entry drifts from the library, CI breaks before you
do. For clingo's own semantics, the
...
breaks the build, not you. First, terms, operators, and negation:
...
assert X.in_(RangePool(1, 5)).render() == "X = 1..5"
assert (X == Y + 1).render() == "X = Y + 1"  # equality binds X: safe
assert SUP.render() == "#sup" and INF.render() == "#inf"
assert ConditionalLiteral(P(x=X), Q(x=X)).render() == "p(X) : q(X)"

# Arithmetic spellings
assert (X // 2).render() == "X / 2"
assert (X % 2).render() == "X \\ 2"
assert (Number(1) | 2).render() == "1 ? 2"
assert Compl(X).render() == "~X"
assert abs(X).render() == "|X|"

# Negation
assert Not(P(x=X)).render() == "not p(X)"
assert (~~P(x=X)).render() == "not not p(X)"  # preserved
assert (~(X != 3)).render() == "X = 3"  # complement, not a "not" wrapper
assert (-P(x=3)).render() == "-p(3)"  # classical: the sign is on the atom
```

Then choices, aggregate guards, and full statement forms through a program:

```python
# Choices and aggregate guards
assert Choice(P(x=X), condition=Q(x=X)).exactly(1).render() == "{ p(X) : q(X) } = 1"
assert (Count(X, condition=P(x=X)) > 3).render() == "#count{ X : p(X) } > 3"
assert (Count(X, condition=P(x=X)) >= 2).render() == "#count{ X : p(X) } >= 2"

# Bare atoms are zero-arity predicates
n = Predicate.define("n", [], show=False)
assert n().render() == "n"

# Full statement forms
program = ASPProgram()
...
assert "#const width = 9." in rendered
assert "p(width)." in rendered
assert "p(X) :- q(X)." in rendered
assert ":- p(X), q(X)." in rendered
assert "{ p(X) : q(X) } = 1 :- q(X)." in rendered
```
