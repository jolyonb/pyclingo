...
we match but never mention. `render()` shows the ASP so far — the facts, then
the two new rules (elided here, printed whole
[further down](#see-the-clingo-you-wrote)):
...
this way, and every transcript above is a doctest: CI executes it and
compares the whole render, character for character, so the ASP on this page
cannot drift from the ASP the library emits.
...
which adds one more line to the program:
...
Solve again, and no color paints more than three states:
...
which renders as a weak constraint — `:~` instead of `:-`, a cost instead of
a veto:
