...
pinned LIVE in tests/aspalchemy/test_scoping.py: every case there carries
...
from aspalchemy.conditioned_element import ConditionedElement
...
if TYPE_CHECKING:
    ...
    from aspalchemy.optimization import OptimizationDirective, WeakConstraint
...
class LocalScope:
    ...
    # head variable ends up unsafe (a deliberate aspalchemy-only lint —
    ...
    for term in terms:
        ...
        if found is not None:
            raise ValueError(
                ...
                f"copy separately for safety, which aspalchemy does not model: "
...
def validate_rule(head: Term | None, body: list[Term], rule: str | Rule, *, check_singletons: bool) -> None:
    ...
    rejections); the singleton check is a aspalchemy-only lint and can be
