...
from aspalchemy.core import Comparison, DefaultNegation, DefinedConstant, Pool, PredicateOccurrence, Term, require_int32
from aspalchemy.exceptions import GroundingError, UnsatisfiableError
...
from aspalchemy.version import __version__
...
class ASPProgram:
    ...
    def __init__(
        ...
        aspalchemy-only — gringo is silent about singletons — so exploratory
    ...
    def _render_lines(self, annotate: bool) -> tuple[list[RenderedLine], set[type[Predicate]], bool]:
        ...
        lines.append(f"% Generated by aspalchemy {__version__}")
    ...
    def ground(self, stop_on_log_level: LogLevel = LogLevel.INFO, context: object = None) -> GroundedProgram:
        ...
        aspalchemy does not model @-terms, so the text and the context must
        agree on their own.
        ...
        if has_raw:
            ...
            if undeclared:
                ...
                raise ValueError(
                    f"The grounded program contains predicates never declared to aspalchemy: "
...
class GroundedProgram:
    ...
    def control(self) -> clingo.Control:
        ...
        risk. aspalchemy's guarantees stop here: direct mutations bypass the
        ...
        between solves, never during one, and prefer the aspalchemy verbs
        where they exist.
        """
