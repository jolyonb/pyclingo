...
from aspalchemy.clingo_handler import ClingoMessage, ClingoMessageHandler
...
from aspalchemy.exceptions import UnsatisfiableError
from aspalchemy.predicate import Predicate
from aspalchemy.statistics import format_statistics_clingo_style
...
def convert_symbol_to_predicate(symbol: clingo.Symbol, predicate_types: PredicateTypes) -> Predicate:
    ...
    if symbol.type != clingo.SymbolType.Function or symbol.name == "":
        raise ValueError(
            ...
            f"(#show expr : condition) emit arbitrary terms, which aspalchemy does not "
    ...
    for i, (arg, field_name) in enumerate(zip(symbol.arguments, field_names, strict=True)):
        if arg.type == clingo.SymbolType.Number:
            ...
            if arg.name == "":
                ...
                raise ValueError(
                    f"Argument {i} of {pred_name} is the clingo tuple {arg}, which aspalchemy "
    ...
    try:
        ...
        raise type(e)(
            ...
            f"aspalchemy has no escaping support.)"
        ) from e
