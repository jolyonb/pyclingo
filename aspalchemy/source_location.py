...
(aspalchemy itself, plus register_skip_package() — a whole package or a
...
# fact() call)
_skip_prefixes: frozenset[str] = frozenset({"aspalchemy"})
...
_override: ContextVar[SourceLocation | None] = ContextVar("aspalchemy_location_override", default=None)
...
def register_skip_package(name: str) -> None:
    ...
    built on aspalchemy registers itself ("myfw") so captured locations point at
